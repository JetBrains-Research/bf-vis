...
export const generateSvgSquare = (size, color) => {
    if (size && color)
        return <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} fill={color} className="bi bi-square-fill" viewBox="0 0 16 16">
...
function LegendSize(props) {
    ...
    return (
        <div id="legend-size-container" className='row panel mt-2 pt-2 pb-2' >
            <h4>Size</h4>
            <p className='small'>We have a log base 2 scale for size. Sizes are relative to other tiles on the same directory-level</p>
