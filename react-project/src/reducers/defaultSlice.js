...
const defaultSlice = createSlice(
    {
        ...
        reducers: {
            stats: (state, action) => {
                ...
                if (action.payload && action.payload.path && action.payload.path !== state.c) {
                ...
                if (newFilter) {
                    if (newFilter.exclude) {
                        ...
                        if (exclude.extensions) {
                            state.filters.exclude.extensions = [...new Set(state.filters.exclude.extensions.concat(exclude.extensions))];
                        }
                    }
                }
            },
                ...
                if (newFilter) {
                    if (newFilter.exclude) {
                        ...
                        if (exclude.fileNamePrefix) {
                            state.filters.exclude.fileNamePrefix = [...new Set(state.filters.exclude.fileNamePrefix.concat(exclude.fileNamePrefix))];
                        }
                    }
                }
            },
                ...
                if (filterToBeRemoved) {
                    if (filterToBeRemoved.exclude) {
                        ...
                        if (excludeToBeRemoved.fileNamePrefix) {
                            state.filters.exclude.fileNamePrefix = state.filters.exclude.fileNamePrefix.filter((element) => !excludeToBeRemoved.fileNamePrefix.includes(element));
                        }
                    }
                }
            },
                ...
                if (filterToBeRemoved) {
                    if (filterToBeRemoved.exclude) {
                        ...
                        if (excludeToBeRemoved.extensions) {
                            state.filters.exclude.extensions = state.filters.exclude.extensions.filter((element) => !excludeToBeRemoved.extensions.includes(element));
                        }
                    }
                }
            },
...
export const { scopeIn, scopeOut, reset, addExtensionExcludeFilter, removeExtensionExcludeFilter, addFileNamePrefixExcludeFilter, removeFileNamePrefixExcludeFilter, stats } = defaultSlice.actions;
export const selectFullData = (state) => state.default.fullData;
export const selectCurrentVisualizationData = (state) => state.default.currentVisualizationData;
export const selectCurrentVisualizationPath = (state) => state.default.currentVisualizationPath;
